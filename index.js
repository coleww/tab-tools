...
var require_dist = __commonJS({
  "dist/index.js"(exports, module2) {
    ...
    function validateTabData(tabData) {
      return tabData.tuning.length && tabData.data.length && tabData.tuning.length === tabData.data.length && allArraysAreSameLength(tabData.data);
    }
    ...
    function getUniqueNotes(tabData) {
      return tabData.data.reduce((bigAcc, string, i) => {
      ...
      }, []).filter((note, i, arr) => arr.indexOf(note) === i).sort();
    }
    function createTab2(tabData) {
      ...
      return (data[0] || []).reduce((res, _, beatIdx) => {
        const columnHasDouble = data.some((stringData) => {
          ...
          return (colContent == null ? void 0 : colContent.length) && (colContent == null ? void 0 : colContent.length) > 1;
        });
        return res.map((stringRes, stringIdx) => {
          ...
          return stringRes += "-" + handleDouble(beatData || "-", columnHasDouble);
        });
    ...
    function getChords(notes) {
      ...
      return notes.sort().filter((x, i, a) => a.indexOf(x) === i).map((_, i, arr) => {
        const noteOrder = [...arr.slice(i), ...arr.slice(0, i)];
        const root = noteOrder[0];
        if (!root)
          return "";
        const intervalsString = noteOrder.slice(1).map((note) => getInterval(root, note)).toString();
        const chordMatch = Object.keys(CHORD_INTERVALS).find(
          (chordType) => {
            var _a;
            return intervalsString === ((_a = CHORD_INTERVALS[chordType]) == null ? void 0 : _a.toString());
          }
        );
    ...
    function getTabChords2(tabData) {
      var _a;
      const { tuning, data } = tabData;
      return (_a = data[0]) == null ? void 0 : _a.map(
        (_, fretIdx) => data.map((string, stringIdx) => getNote(tuning[stringIdx], string[fretIdx])).filter((x) => x)
      ).map(getChords);
    }
  }
});
...
document.body.addEventListener("paste", function(e) {
  ...
  ;
  try {
    const data = ((_b = (_a = e.clipboardData) == null ? void 0 : _a.getData("text/plain")) == null ? void 0 : _b.split("\n")) || [];
    ...
    const fmtJson = (0, import_json_nice.default)(tabData, { indent: " " }).split("\n   ").join(" ") || "";
